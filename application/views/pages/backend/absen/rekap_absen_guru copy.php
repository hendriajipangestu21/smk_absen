		...
		<table class="table table-bordered table-hover">
			...
			<tbody>
				...
				foreach ($contents["data_rekap"] as $key => $value) :
					...
					$tanggal_waktu_masuk = date('D', strtotime($value->date));
					$listHari = array(
						'Sun' => 'Minggu',
						'Mon' => 'Senin',
						'Tue' => 'Selasa',
				...
				?>
					<tr>
						...
						<td><?php
							$tanggal_masuk = date('D', strtotime($value->date));
							$dayList = array(
								'Sun' => 'Minggu',
								'Mon' => 'Senin',
								'Tue' => 'Selasa',
							...
							echo $dayList[$tanggal_masuk] . ", " . date_format(date_create($value->date), 'd-m-Y');
						...
						if ($value->time > $jam_masuk_tambah) {
							...
							if ($dayList[$tanggal_masuk] == "Senin") {
								$masuk = new DateTime($value->jam_masuk1);
							} elseif ($dayList[$tanggal_masuk] == "Selasa") {
								$masuk = new DateTime($value->jam_masuk2);
							} elseif ($dayList[$tanggal_masuk] == "Rabu") {
								$masuk = new DateTime($value->jam_masuk4);
							} elseif ($dayList[$tanggal_masuk] == "Kamis") {
								$masuk = new DateTime($value->jam_masuk5);
							} elseif ($dayList[$tanggal_masuk] == "Jumat") {
								$masuk = new DateTime($value->jam_masuk6);
							} elseif ($dayList[$tanggal_masuk] == "Sabtu") {
								$masuk = new DateTime($value->jam_masuk7);
							}
						...
						<?php if ($value->jam_pulang1 > $value->time_out) {
