	...
	<div class="card-body">
		<form action="<?= base_url("admin/siswa/update") ?>" method="post" enctype="multipart/form-data">
			<div class="form-group">
				...
				<input type="text" name="nis" class="form-control" value="<?= $contents["siswa"]->nis ?>" required autofocus>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<select name="kelas_id" class="form-control select2" id="" required>
					<?php foreach ($kelas as $value) : ?>
						<option value="<?= $value->kelas_id ?>" <?= $contents["siswa"]->kelas_id == $value->kelas_id ? "selected" : "" ?>><?= $value->kode_kelas . "/" . $value->nama_kelas ?></option>
					<?php endforeach; ?>
				</select>
			</div>
			<div class="form-group">
